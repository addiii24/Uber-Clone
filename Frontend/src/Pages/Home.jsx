...
const Home = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (pickup && dropoff) {
      console.log('Finding trip:', { pickup, dropoff })
    }
  }
  ...
  return (
    <div className="h-screen w-full max-w-md mx-auto relative overflow-hidden bg-gray-100 font-['Inter',sans-serif]">
      ...
      <div
        ref={panelRef}
        className={`absolute left-0 right-0 z-30 bg-white rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.12)] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
          panelOpen ? 'bottom-0 h-[75%]' : 'bottom-0 h-auto'
        }`}
      >
        {/* Drag handle */}
        <div
        ...
        <div className="px-5 pb-6">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* Find trip button — visible when panel open */}
            {panelOpen && pickup && dropoff && (
              <button
                type="submit"
                className="w-full mt-4 bg-black text-white font-semibold py-3.5 rounded-xl text-base hover:bg-gray-900 active:scale-[0.98] transition-all"
              >
                Find Trip
              </button>
            )}
