...
const Lookingforvehicle = ({ ride, pickup, dropoff, onCancel, onDriverFound }) => {
    ...
    <div className="animate-[fadeIn_0.3s_ease]">
      ...
      <div className="flex items-center justify-between bg-gray-50 rounded-2xl px-4 py-3 mb-6">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-sm text-gray-600">4.2 km</span>
        </div>
        <div className="h-4 w-[1px] bg-gray-300" />
        <div className="flex items-center gap-1">
          <span className="text-sm font-bold text-gray-900">₹{ride.price}</span>
          {ride.originalPrice && (
            <span className="text-xs text-gray-400 line-through">₹{ride.originalPrice}</span>
          )}
        </div>
      </div>
